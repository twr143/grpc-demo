...
class EndpointsToRoutes(http: Http, apiContextPath: List[String]) {
  ...
  private val decodeFailureHandler: DecodeFailureHandler = {
    def failResponse: (StatusCode, List[Header], String) => ValuedEndpointOutput[_] =
      (c, hs, m) => ValuedEndpointOutput(headers.and(http.failOutput), (hs, c, Error_OUT(List(m))))
  ...
  def toDocsRoutes(es: ServerEndpoints): HttpRoutes[IO] = {
    val swaggerEndpoints =
      SwaggerInterpreter(basePrefix = apiContextPath, customiseDocsModel = _.addServer(s"/${apiContextPath.mkString("/")}"))
