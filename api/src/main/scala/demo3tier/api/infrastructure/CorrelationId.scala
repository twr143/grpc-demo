...
import demo3tier.api.util.{CorrelationIdDecorator, Http4sCorrelationMiddleware}
...
/** An sttp backend wrapper, which sets the current correlation id on all outgoing requests.
  */
case class SetCorrelationIdBackend[P](delegate: SttpBackend[IO, P]) extends SttpBackend[IO, P] {
